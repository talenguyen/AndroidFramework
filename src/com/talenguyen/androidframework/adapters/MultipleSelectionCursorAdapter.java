...
public abstract class MultipleSelectionCursorAdapter<T> extends CursorAdapter {
	...
	private List<Integer> selectedIndexs; // The array which will contain the
											// position of the selections.
	private Class<? extends T> clazz;

	public MultipleSelectionCursorAdapter(Context context,
	 ...
	 * 
	 * @param position
	 *            The position where item is toggleSelect
	 */
	...
	public List<T> getSelectedItems() {
		...
		for (int i = 0; i < length; i++) {
			...
			if (cursor.moveToPosition(index)) {
				T item;
				try {
					item = SQLiteHelper.fromCursor(cursor, clazz);
					if (item != null) {
						result.add(item);
					}
	 ...
	 * @return <b>true</b> if the item at position is selected or <b>false</b>
	 *         otherwise.
	 */
	public boolean isSelected(int position) {
		if (selectedIndexs != null
				&& selectedIndexs.contains(Integer.valueOf(position))) {
			return true;
		}
		return false;
	}
