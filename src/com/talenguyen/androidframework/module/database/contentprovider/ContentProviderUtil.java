...
public class ContentProviderUtil {
    ...
    /**
     * Deletes row(s) specified by a content URI. If the content provider supports transactions, the
     * deletion will be atomic.
     *
     * @param tableClass    The table to be deleted
     * @param where         A filter to apply to rows before deleting, formatted as an SQL WHERE
    ...
    public static int delete(Class<? extends ITable> tableClass, String where,
        ...
        verify();
        return sContentResolver.delete(getUri(tableClass), where, selectionArgs);
    }

    /**
     * Inserts a row into a table. If the content provider supports transactions the insertion will be atomic.
     ...
     * @throws Throws NullPointerException if uri or values are null
     */
    public static int update(ITable item) {
        ...
        final ContentValues values = SQLiteHelper.buildContentValue(item);
        return sContentResolver.update(getUri(item.getClass()), values, "_id LIKE ?",
    ...
    /**
     * Query the given URI, returning a Cursor over the result set. For best performance, the caller
     * should follow these guidelines:    Provide an explicit projection, to prevent reading data
     * from storage that aren't going to be used.     Use question mark parameter markers such as
     * 'phone=?' instead of explicit values in the selection parameter, so that queries that differ
     ...
     */
    public static Cursor query(Class<? extends ITable> tableClass, String selection,
